...
# Try legacy/newer RLlib locations; else define our own.
try:
    from ray.rllib.models.misc import normc_initializer          # some 0.8.x builds
except Exception:
    try:
        from ray.rllib.models.tf.misc import normc_initializer   # present in 1.x / some 0.8.x wheels
    except Exception:
        import tensorflow as tf
        def normc_initializer(std=1.0):
            def _initializer(shape, dtype=None, partition_info=None):
                w = tf.random.normal(shape, dtype=dtype)
                w *= std / tf.sqrt(tf.reduce_sum(tf.square(w), axis=0, keepdims=True))
                return w
            return _initializer
